...
with open('./good_creative_birds.pkl', 'rb') as f:
    # This pkl is simply the original dataset filtered to keep only "good" images
    data = pickle.load(f)
...
raster_images_hq = vector_to_raster(data, part_label=False, nodetail=True, side=size*4, line_diameter=3, padding=16, bg_color=(1, 1, 1), fg_color=(0, 0, 0))

for i, (img, img_rgb, img_hq) in enumerate(tqdm(zip(raster_images, raster_images_rgb, raster_images_hq))):
